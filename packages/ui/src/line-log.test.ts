...
describe("createLineLogger", () => {
  it("renders one line for human-meaningful events", () => {
    ...
    expect(line({ type: "run-started", runId: "r1" as RunId, name: "demo", at: 0 })).toBe(
      "▶ demo (r1)",
    );
    ...
    ).toBe("  ✗ k0: BudgetExhausted: spent 5 of 5");
    expect(
      line({
        type: "agent-failed",
        key: "k1",
        error: { kind: "AdapterSpawn", adapter: "claude", cause: "exit 1: boom" },
        at: 0,
      }),
    ).toBe("  ✗ k1: AdapterSpawn (claude): exit 1: boom");
  ...
  it("renders a question and its answer", () => {
    ...
    ).toBe("? deploy-target: ## Where to deploy?");
    expect(
      line({
  ...
  it("resolves the friendly label from agent-queued instead of the internal key", () => {
    ...
    expect(
      line({
        type: "agent-finished",
        key,
        usage: { inputTokens: 1, outputTokens: 721 },
        cached: false,
        at: 0,
      }),
    ).toBe("  ✓ draft:release (721 tok)");
  });
