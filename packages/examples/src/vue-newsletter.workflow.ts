...
//
// `meta.harness` is required and is the single source of truth for which agent
// CLI runs each step — here it's "claude" (Claude Code), so every agent() is a
// real `claude -p` invocation with web search. This spawns real agents and uses
// tokens.
//
// `z` is injected into the sandbox by the runtime (the same zod instance the
// engine uses to convert/validate). Build a schema with it and pass it as
// `agent(prompt, { schema })` — the agent is then forced to return JSON matching
// the schema, and agent() resolves to the validated, typed object instead of raw
// text. No imports: `agent`, `parallel`, `phase`, `log`, `args`, and `z` are all
...
// One agent researches each source, all returning the same shape.
const ITEM = z.object({
  title: z.string(),
  url: z.string(),
...
const raw = await parallel(
  SOURCES.map((s) => () =>
    agent(
      ...
      { label: `research:${s.key}`, phase: "Research", schema: SOURCE_RESULT },
    ),
  ),
);

// `schema` makes each agent() resolve to a typed SOURCE_RESULT (or null if that
