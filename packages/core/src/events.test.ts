...
describe("event reducer", () => {
  ...
  it("sets endedAt on agent-failed", () => {
    const events: WorkflowEvent[] = [
      ...
      { type: "agent-started", key: "0", at: 1 },
      {
        type: "agent-failed",
        key: "0",
        error: { kind: "AdapterSpawn", adapter: "x", cause: "boom" },
        at: 5,
      },
    ];
    const state = events.reduce(reduce, initialRunState());
  ...
  it("flags approximate token usage on the agent", () => {
    const events: WorkflowEvent[] = [
      { type: "agent-queued", key: "0", label: "a", phase: "P", at: 0 },
      {
        type: "agent-finished",
        key: "0",
        usage: { inputTokens: 1, outputTokens: 2, approximate: true },
        cached: false,
        at: 1,
      },
    ];
  ...
  it("preserves the error on agent-failed", () => {
    ...
    expect(state.agents.get("0")?.error).toEqual({
      kind: "AdapterSpawn",
      adapter: "claude",
      cause: "exit 1: boom",
    });
